...
public class HabitLogController {
    ...
    public ResponseEntity<?> createNewHabitLog(@RequestBody HabitLogRequest habitLogRequest) {
        ...
        HabitLogEntity habitLogEntity = habitLogService.createNewHabitLogService(habitLogRequest);
    ...
    @GetMapping("/{habit-id}")
    public ResponseEntity<?> getHabitLogById(@PathVariable("habit-id") UUID habitId, @RequestParam(defaultValue = "1") Integer page, @RequestParam(defaultValue = "3") Integer size) {
        List<HabitLogEntity> getteHabitLogById = habitLogService.getHabitLogByIdService(habitId, page, size);
